 ...
 */
class ClosureDiff implements Diff{

    private BiMap<String,String> namespaceMap;
    private Model m1, m2;
    static final Diff INSTANCE = new ClosureDiff();
    private Closure closureClass1,closureClass2,closureProp1,closureProp2;
    private Map<RdfNode,Node> classMap1,classMap2,propMap1,propMap2;
    private Delta delta;
    ...
    public Delta diff(Model m1, Model m2, BiMap<String,String> mapping) {
        ...
        g = GraphUtils.toGraph(m2, RdfSchema.SUBCLASSOF);
        for(RdfNode n: m2.findNodes(RdfType.CLASS)){
            if(!g.nodeMap().containsKey(n)){
                g.nodeMap().put(n, g.graph().newNode(n));
            }
        }
        for(RdfNode n: m2.findNodes(RdfType.METACLASS)){
            if(!g.nodeMap().containsKey(n)){
                g.nodeMap().put(n, g.graph().newNode(n));
            }
        }
        for(RdfNode n: m2.findNodes(RdfType.METAPROPERTY)){
            if(!g.nodeMap().containsKey(n)){
                g.nodeMap().put(n, g.graph().newNode(n));
            }
        }
        classMap2 = g.nodeMap();
        closureClass2 = Transitivity.acyclicClosure(g.graph(),
            SuccessorSetFactory.intervalBased(g.graph()));
        ...
        g = GraphUtils.toGraph(m2, RdfSchema.SUBPROPERTYOF);
        for(RdfNode n: m2.findNodes(RdfType.PROPERTY)){
            if(!g.nodeMap().containsKey(n)){
                g.nodeMap().put(n, g.graph().newNode(n));
            }
        }
        propMap2 = g.nodeMap();
        closureProp2 = Transitivity.acyclicClosure(g.graph(),
                SuccessorSetFactory.intervalBased(g.graph()));
    ...
    private Delta compareTriples() {
        ...
        for(Triple triple1: m1.triples().fetch()){
            ...
            if(triple1.predicate().getUri().equals(Rdf.TYPE)){
                ...
                Resource mappedP = (Resource)DiffUtils.mapNodeToAnotherModel(triple1.predicate(), m2, namespaceMap);
        ...
        for(Triple triple2: m2.triples().fetch()){
            ...
            if(triple2.predicate().getUri().equals(Rdf.TYPE)){
                if(!superTypes.contains(triple2.subject().getIdentifier())){
                    superTypes.add(triple2.subject().getIdentifier());
                    Set<RdfNode> st = DiffUtils.getSuperTypes(
                            m2,(Resource)triple2.subject(),closureClass2,classMap2);
                    if(mappedS == null ){
                        for(RdfNode node: st){
                    ...
                    DiffUtils.findAddedNeighboors(m1,m2,(Resource)mappedS,
                            (Resource)triple2.subject(),triple2.predicate(),st,namespaceMap,delta);
                }
            }else if(triple2.predicate().getUri().equals(RdfSchema.SUBCLASSOF)){
                if(!ancestors.contains(triple2.subject().getIdentifier())){
                    ancestors.add(triple2.subject().getIdentifier());
                    Set<RdfNode> an = DiffUtils.getAncestors(
                            m2,(Resource)triple2.subject(),closureClass2,classMap2);
                    if(mappedS == null ){
                        for(RdfNode node: an){
                    ...
                    DiffUtils.findAddedNeighboors(m1,m2,(Resource)mappedS,
                            (Resource)triple2.subject(),triple2.predicate(),an,namespaceMap,delta);
                }
            }else if(triple2.predicate().getUri().equals(RdfSchema.SUBPROPERTYOF)){
                if(!ancestors.contains(triple2.subject().getIdentifier())){
                    ancestors.add(triple2.subject().getIdentifier());
                    Set<RdfNode> an = DiffUtils.getAncestors(
                            m2,(Resource)triple2.subject(),closureProp2,propMap2);
                    if(mappedS == null ){
                        for(RdfNode node: an){
                            ((TripleDelta)delta).insertAddedTriple(
                                    triple2.subject(),
                ...
                if(triple2.object().isResource()){
                    mappedO = DiffUtils.mapNodeToAnotherModel((Resource)triple2.object(), m1, namespaceMap);
                }else if(triple2.object().isLiteral()){
                    if(m1.map(triple2.object().toString()).hasTriples()){
                        mappedO = m1.map(triple2.object().toString());
                    }
                }
                Resource mappedP = (Resource)DiffUtils.mapNodeToAnotherModel(triple2.predicate(), m1, namespaceMap);
