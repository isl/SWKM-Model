 ...
 * @author egiannak
 */
public class TestImporterSaveISA extends TestCase {
        private final String name;
        private final Representation repr;
        String[][] paths = {
            {"http://ngtest.org#", "test/resources/TestClassSubAnc.trig"}
        };
        protected final Jdbc jdbc;
        private final Database db = Database.db1;

        public TestImporterSaveISA(String testName, Representation repr,
            String name) {
        super(testName);
        this.repr = repr;
        this.name = name;
...
//        for (Representation r : Representation.values()) {
        for (Representation r : Arrays.asList(Representation.HYBRID)) {
            suite.addTest(new TestImporterSaveISA("test", r, "test schema"));
        }
        return suite;
    }
    ...
    private void storeNew(Database db, Representation repr) throws Exception {
        ...
        for (int i = 0; i < paths.length; i++) {
            ...
            System.out.println("Model before the importer: " +  model);
            store.store(model);
            System.out.println("Model after the importer: " + model);
